...
![Подтверждение номера кодом из SMS](docs/screenshots/widget-user-03-code.png)
...
Maps доступны как альтернатива.

![Подробности обезличенного варианта](docs/screenshots/widget-user-05-details.png)

![Успешный запрос специалисту](docs/screenshots/widget-user-06-request-sent.png)
...
открытии виджет сверяет с сервером только их анонимные коды `LP-…`. Снятая с
публикации карточка остаётся в списке для истории, получает заметную отметку и
больше не позволяет отправить запрос специалисту.
...
Закрытый раздел показывает подтверждённые контакты, условия поиска и точное
исходное объявление только после явного запроса посетителя.

![Очередь обращений из виджета](docs/screenshots/widget-admin-01-requests.png)

![Исходное объявление и история для менеджера](docs/screenshots/widget-admin-02-listing.png)

![Панель менеджера на телефоне](docs/screenshots/widget-admin-03-mobile.png)
...
7. Если нужна помощь, нажимает **«Попросить специалиста проверить»** в деталях
   варианта. Только после этого менеджер получает точное исходное объявление,
   параметры текущего поиска и подтверждённый телефон.
...
- подтверждённый телефон, страницу входа и UTM-метки;
- возможность предложить оценку участка, проект дома и строительство;
- очередь запросов по конкретным объявлениям в отдельном разделе **«Виджет»**;
- передачу подтверждённого лида и запроса консультации в CRM через webhook;
...
- сервер получает только анонимные коды сохранённых карточек для проверки их
  актуальности;
- сохранение и сравнение не создают заявку менеджеру;
...
Даже после подтверждения публичный API не отдаёт оригинальные заголовки,
описания, фотографии, источники, внутренние ID и ссылки. Координаты округляются,
а каждый вариант получает код вида `LP-A1B2C3D4E5`. Исходная ссылка и точные
данные остаются в закрытой панели менеджера.
