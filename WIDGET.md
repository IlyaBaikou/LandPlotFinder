...
В карточке есть ссылка на исходное объявление с фотографиями. Кнопка запроса
консультации удалена.
...
Закрытый раздел показывает номера посетителей, которые ввели код. В демо-режиме
это не подтверждает владение номером. Отправка лидов в Telegram менеджеру пока
не подключена.
...
- список посетителей в отдельном разделе **«Виджет»**;
