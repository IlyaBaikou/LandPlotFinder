...
- macOS/Linux: дважды открыть `start.command` или выполнить `./start.command`;
- Windows: дважды открыть `start.bat`.

Приложение само соберётся и откроет `http://127.0.0.1:8787`. При первом запуске
появится мастер настройки критериев и расписания. Данные сохраняются между
обновлениями и перезапусками.
...
планирования поездки по приглянувшимся участкам.
