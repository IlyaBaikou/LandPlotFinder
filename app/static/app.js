...
function widgetClientCard(lead) {
  ...
  const telegramStatus = ({ active: 'Telegram: подписан', paused: 'Telegram: пауза', stopped: 'Telegram: отписался' })[lead.telegram_status] || '';
  return `<article class="widget-client-card"><div class="widget-client-top"><div><span class="widget-ref">КЛИЕНТ #${esc(lead.id)}</span><h3>${phoneMarkup}</h3><small>Код введён ${esc(dt(lead.verified_at))}</small>${telegramStatus ? `<small>${esc(telegramStatus)}</small>` : ''}</div><span class="widget-client-status ${interests.length ? 'interested' : ''}">${esc(status)}</span></div><div class="widget-client-search"><div class="widget-subhead"><strong>Последний поиск</strong><time>${lead.search_updated_at ? esc(dt(lead.search_updated_at)) : 'Пока не выполнялся'}</time></div><div class="widget-criteria">${widgetCriteria(lead.search || {})}</div></div>${interests.length ? `<div class="widget-client-interests"><div class="widget-subhead"><strong>Интересные варианты</strong><span>Отмечены самим посетителем</span></div>${interests.map(widgetInterestCard).join('')}</div>` : ''}</article>`;
}
