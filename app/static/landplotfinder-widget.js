(function () {
  ...
  function listingLink(item, unavailable = false, inDetail = false) {
    ...
    return `<a class="${inDetail ? "lpf-source-link lpf-source-detail" : "lpf-source-link"}" href="${url}" target="_blank" rel="noopener noreferrer">${inDetail ? "Посмотреть фото и объявление ↗" : "Открыть объявление ↗"}</a>`;
  }
